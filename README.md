...
6. **Legal Authority:** The script attempts to respect `robots.txt` automation permissions (skipping URLs that are explicitly disallowed) and adds MIT license attribution at the top of each generated category file. It may proceed if `robots.txt` cannot be fetched, so you should still perform your own legal review for your specific use case.
